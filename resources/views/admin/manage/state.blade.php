...
<div class="page-breadcrumb">
    <div class="row align-items-center">
        <div class="col-md-6 col-8 align-self-center">
            <h3 class="page-title mb-0 p-0">State</h3>
                ...
                <div class="card-body">
                    <div class="row mb-3">
                        ...
                        <div class="align-right">
                            <div class="text-end upgrade-btn mb-3">
                                <a href="javascript:void(0)" class="btn btn-success link-btn add-native-place">+ Add State</a>
                            </div>
                        </div>
                        @endif
                    </div>
                    ...
                    <table class="surname-datatable table">
                        ...
                        <tbody class="surname_datatable_tbody">
                            ...
                            <tr>
                                <td>{{ $row->id }}</td>
                                <td>{{ (!is_null($row->country)) ? $row->country->name : "<span class='text text-danger'>Deleted</span>" }}</td>
                                <td>{{ $row->name }}</td>
                                <td>
                                    <span class='action'>
                                        @if(Auth::user()->is_update)
                                        <a href='javascript:void(0)' class='edit' data-id='{{ $row->id }}' data-country_id='{{ $row->country_id }}' data-name='{{ $row->name }}'><i class='fa-solid text-success fa-pen-to-square'></i></a>&nbsp;
                                        @endif
                                        @if(Auth::user()->is_delete)
                                        <a href="{{ url('/admin/manage/delete/state/'.$row->id) }}" onclick='return confirm("Are you sure?")'><i class='fa-solid fa-trash text-danger'></i></a>&nbsp;
                                        @endif
                                    </span>
                                </td>
                            </tr>
...
<div class="modal fade" id="modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <form action="{{route('manage.state.store')}}" method="post">
        @csrf
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="country_id" id="country_id" class="form-control select" style="width: 100%;"></select>
                    </div>
                    <div class="mb-3">
...
<script>
    $(document).ready(function(){
        ...
        $("#custom_search").on("keyup", function () {
            ...
            $.ajax({
                url: "{{ route('manage.state.ajax_search') }}",
                method: "POST",
                data: {
                    _token : "{{ csrf_token() }}",
                    search: search_title
        ...
        var table = $('.surname-datatable_____asdad').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                "url": "{{ route('manage.state.list') }}"
            },
            columns: [
                {
    ...
    function country(options_element, selected_value = null) {
        let cityUrl = "{{ route('manage.country.list') }}?ajax";
